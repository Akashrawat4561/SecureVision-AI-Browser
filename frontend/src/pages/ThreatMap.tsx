...
type AttackType = 'Phishing' | 'Ransomware' | 'DDoS' | 'Malware' | 'Credential Harvest' | 'Zero-Day' | 'Spear Phishing' | 'Botnet';
...
const GEO_POOL: { city: string; country: string; coords: [number, number] }[] = [
...
let _nextId = 1;
function makeEvent(): ThreatEvent {
    const geo  = GEO_POOL[Math.floor(Math.random() * GEO_POOL.length)];
    const sevs: Severity[] = ['critical','high','high','medium','medium','medium','low','low'];
    return {
        id:          _nextId++,
...
export default function ThreatMapPage() {
    const [events, setEvents]         = useState<ThreatEvent[]>(() => Array.from({ length: 12 }, makeEvent));
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-2 text-brand-cyan">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] font-mono">
                            Intelligence Stream // Global Threat Surface
                        </span>
                    </div>
                    ...
                    <p className="text-slate-500 text-sm font-medium max-w-lg">
                        Real-time geospatial telemetry of active cyberattack vectors — dynamically updated every few seconds.
                    </p>
                </div>
            ...
            <div className="glass-panel overflow-hidden relative" style={{ height: '520px' }}>
                ...
                <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-8 py-5 bg-gradient-to-b from-slate-950 via-slate-950/80 to-transparent pointer-events-none">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest font-mono">Global Threat Surface</div>
                            <div className="text-xs font-bold text-white font-mono">{filtered.length} active vector{filtered.length !== 1 ? 's' : ''} visible</div>
                        </div>
                    </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 px-8 py-4 bg-gradient-to-t from-slate-950 to-transparent pointer-events-none flex items-end justify-between">
                    ...
                    <div className="text-[9px] font-mono text-slate-700 uppercase tracking-widest">
                        PROJ: Equirectangular | GRID: 15° | VEC: {filtered.length}
                    </div>
                </div>
                    ...
                    <div className="flex items-center justify-between mb-6 shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold">Real-time ingress — auto-refreshing</p>
                            </div>
                        </div>
                        ...
                        <table className="w-full text-[11px]">
                            <thead className="sticky top-0 bg-slate-950/95 backdrop-blur-sm z-10">
                                <tr className="border-b border-white/5">
                                    {['Severity','City / Country','Attack Vector','Source IP','Pkts','Status','When'].map(h => (
                                ...
                                <AnimatePresence initial={false}>
                                    {filtered.map((ev) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-3 text-slate-700 font-mono text-[9px] whitespace-nowrap" suppressHydrationWarning>
